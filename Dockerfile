...
RUN set -eu; \
    (while true; do echo "[casadf-build] vite build em andamento..."; sleep 20; done) & \
    HEARTBEAT_PID=$!; \
...
RUN set -eu; \
    (while true; do echo "[casadf-build] esbuild backend em andamento..."; sleep 20; done) & \
    HEARTBEAT_PID=$!; \
...
RUN set -eu; \
    (while true; do echo "[casadf-build] instalando chromium/dependencias..."; sleep 20; done) & \
    HEARTBEAT_PID=$!; \
    apt-get update; \
...
RUN set -eu; \
    (while true; do echo "[casadf-build] preparando pnpm runtime..."; sleep 20; done) & \
    HEARTBEAT_PID=$!; \
    corepack enable; \
...
RUN --mount=type=cache,id=pnpm-store-prod,uid=1000,target=/home/node/.local/share/pnpm/store \
    set -eu; \
    (while true; do echo "[casadf-build] pnpm install prod em andamento..."; sleep 20; done) & \
    HEARTBEAT_PID=$!; \
